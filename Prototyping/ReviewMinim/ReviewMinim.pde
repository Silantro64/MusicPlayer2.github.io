...
void setup() {
  ...
  String musicDirectory = upArrow + upArrow + upArrow + musicFolder; //Concatenation
  String soundEffectsDirectory = upArrow + upArrow + upArrow + soundEffectsFolder; //Concatenation
  //
  String pathway;
  ...
  for (AudioPlayer song : playList) {
    if ( song == null) { //ERROR, play list is NULL
      ...
      println("The Playlist didn't load properly");
      printArray(playList);
      exit();
    }
  }//end playlist error check
  //
  if (soundEffects[currentSong] == null) {
    //See FILE or minim.loadFile
    println("the sound effects didn't load properly");
